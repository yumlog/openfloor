...
const SLIDE_SCALE_OUT: number[] = [0, 1.0]
...
function PortfolioText({
  ...
  // 정방향(opacity→0)은 TEXT_FADE_DURATION, 역방향(opacity→1)은 데스크탑에서만 더 짧게.
  const reverseDur = isMobile ? TEXT_FADE_DURATION : TEXT_FADE_REVERSE_DURATION
  const fade = {
    initial: false as const,
    animate: { opacity: textHidden ? 0 : 1 },
    transition: {
      duration: textHidden ? TEXT_FADE_DURATION : reverseDur,
