...
/* ---------------------------------------------------------------------------
   Philosophy 스택 — 데스크탑. 트랩 progress(0..1)로 카드가 위로 쌓인다.
     진입(progress 0): 카드가 세로 리스트로 펼쳐짐(간격 20px) — 첫 카드가 보이고
                       나머지는 아래에 대기.
     스크롤(→STACK_END): 각 카드가 이전 카드와 140px 차이로 겹쳐 쌓인다.
   마지막(빨강) 카드가 다 쌓이면 PhilosophyGrow(포털)가 확대를 이어받는다.
   모든 값 design px(1440 기준); 부모 stage가 scale(ratio).
--------------------------------------------------------------------------- */
...
const STACK_END = 0.75 // = GROW_START
    ...
    >
      ...
      >
        ...
        <div className="px-[40px] pt-[48px]">
          ...
          <p className="mt-[74px] text-[24px] leading-[1.6] font-bold whitespace-pre-line text-white">
            {card.body}
          </p>
        </div>
      </motion.div>
...
export function PhilosophyStack({
  ...
  return (
    <div
      ...
      style={{ width: CARD_W, height: STAGE_H, transform: `scale(${ratio})` }}
    >
...
function CardCell({
  ...
  // 펼침(top i*(CARD_H+GAP)) → 쌓임(top i*STACK_OFFSET), progress 0..STACK_END.
  const top = useTransform(progress, (p) => {
    const pN = clamp01(p / STACK_END)
    return (
      STACK_OFFSET * index +
      (1 - pN) * (CARD_H + GAP - STACK_OFFSET) * index
    )
  })

  return (
    // 바깥 = 쌓임 위치(top) + z. z는 바깥에 둬야 형제 순서가 잡힌다(안쪽 transform이
    // stacking context를 만들기 때문).
    <motion.div
